...
def generate_json(json_schema, args=None, remove_null=False, remove_empty=False, default_array_items=1):
    ...
    if type(schema) is dict:
        has_value, result = _generate_object(schema, args, remove_null, False, remove_empty, int(default_array_items))
    elif type(schema) is list:
        has_value, result = _generate_array(schema[0], args, remove_null, False, remove_empty, int(default_array_items))
...
def random_future_datetime(format="%Y-%m-%dT00:00:00Z", daynum=0):
    from datetime import datetime
    from dateutil.relativedelta import relativedelta

    datetime.now().strftime(format)
    date_after = datetime.now() + relativedelta(days=daynum)
    return  date_after.strftime(format)
    ...
    if key is not None:
        ...
        if value is not None:
            ...
            if value_type in TYPE_STRINGS:
                ...
                return True, str(value).encode("utf-8")
...
def _generate_json(schema, args, remove_null, remove_empty, default_array_items, json_path):
    if isinstance(schema, STRING_TYPE):
        ...
        return _generate_object(schema, args, remove_null, remove_empty, remove_empty, default_array_items, json_path)
    elif type(schema) is list:
        return _generate_array(schema[0], args, remove_null, remove_empty, remove_empty, default_array_items, json_path)
    return False, None


def _generate_array(schema, args, remove_null, remove_empty, child_remove_empty, default_array_items, json_path='$'):
    ...
    for index in range(0, size):
        ...
        has_value, value = _generate_json(schema, args, remove_null, child_remove_empty, default_array_items, path)
...
def _generate_object(schema, args, remove_null, remove_empty, child_remove_empty, default_array_items, json_path='$'):
    ...
    result = {}
    for key, schema in schema.iteritems():
